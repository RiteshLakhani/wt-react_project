...
function FacultyList() {
    ...
    let formattedFaculties = faculties.map((fac,index) => {
        return (
                <div class="card col-3">
                    <img src={fac.FacultyImage} class="card-img-top" alt="..."/>
                    <div class="card-body">
                        <h5 class="card-title">{fac.FacultyName}</h5>
                        <p class="card-text">{fac.FacultyDesignation}<br/>
                                             {fac.FacultyExperience}<br/>
                                             {fac.FacultyEducationQualification}<br/>
                                             {fac.FacultyWorkingSince}
                        </p>
                        <a href="https://www.darshan.ac.in/engineering/computer/faculty" class="btn btn-primary">Go somewhere</a>
                        <button class="btn btn-danger m-1" onClick={()=>deleteFac(index)}>Delete Faculty</button>
                    </div>
                </div>
        );
    });

    return <div className="row">{formattedFaculties}</div>
}
